(function() {
  var Parser, alt, alternatives, grammar, name, o, operators, token, tokens, unwrap, _i, _i2, _len, _len2, _ref, _results;
  ...
  grammar = {
    ...
    Operation: [
      ...
      }), o('Expression MERGE Expression', function() {
        return new Merge($1, $3);
      }), o('Expression MERGE Expression WHEN Code', function() {
        return new Merge($1, $3, $5);
      })
    ]
  };
  operators = [['left', '.', '?.', '::'], ['left', 'CALL_START', 'CALL_END'], ['nonassoc', '++', '--'], ['left', '?'], ['right', 'UNARY'], ['left', 'MATH'], ['left', '+', '-'], ['left', 'SHIFT'], ['left', 'RELATION'], ['left', 'COMPARE'], ['left', 'LOGIC'], ['nonassoc', 'INDENT', 'OUTDENT'], ['right', '=', ':', 'COMPOUND_ASSIGN', 'RETURN', 'THROW', 'EXTENDS'], ['right', 'FORIN', 'FOROF', 'BY', 'WHEN'], ['right', 'MERGE'], ['right', 'IF', 'UNLESS', 'ELSE', 'FOR', 'WHILE', 'UNTIL', 'LOOP', 'SUPER', 'CLASS'], ['right', 'POST_IF', 'POST_UNLESS']];
  tokens = [];
  for (name in grammar) {
