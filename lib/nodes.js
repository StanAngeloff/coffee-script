(function(){
  ...
  exports.UsingNode = (function() {
    UsingNode = function UsingNode(properties, expression) {
      this.children = [(this.properties = properties), (this.expression = expression)];
      return this;
    };
    __extends(UsingNode, BaseNode);
    UsingNode.prototype.compile_node = function compile_node(o) {
      var _b, _c, _d, _e, alias, assigns, cached, expression, has_children, is_string, last, nested, property, type;
      assigns = [];
      expression = this.expression.compile(o);
      cached = this.expression instanceof ValueNode && !this.expression.has_properties() ? expression : o.scope.free_variable();
      _c = this.properties;
      for (_b = 0, _d = _c.length; _b < _d; _b++) {
        property = _c[_b];
        ...
        if ((typeof (_e = property.alias) !== "undefined" && _e !== null)) {
          ...
          if (!(last instanceof LiteralNode)) {
            throw new Error(("cannot determine property alias for '" + (property.compile(o)) + "'"));
          }
      ...
      return (cached === expression ? ("" + this.tab) : ("" + (this.tab) + cached + " = " + expression + ";\n" + this.tab)) + assigns.join(("\n" + this.tab));
    };
    return UsingNode;
  })();
