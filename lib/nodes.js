(function() {
  var Access, Arr, Assign, Base, Call, Class, Closure, Code, Comment, Existence, Expressions, Extends, For, IDENTIFIER, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, NEGATE, NO, Obj, Op, Param, Parens, Push, Range, Return, SIMPLENUM, Scope, Slice, Splat, Switch, TAB, THIS, TRAILING_WHITESPACE, Throw, Trait, Try, UTILITIES, Value, While, YES, compact, del, ends, extend, flatten, last, merge, multident, starts, unfoldSoak, utility, _ref;
  ...
  exports.Extends = Extends = function() {
    ...
    function Extends(child, parent, traits) {
      this.child = child;
      this.parent = parent;
      this.traits = traits;
    }
    Extends.prototype.children = ['child', 'parent', 'traits'];
    Extends.prototype.compile = function(o) {
      var code, impl, _i, _len, _ref;
      utility('hasProp');
      code = [];
      if (this.parent) {
        code.push(new Call(new Value(new Literal(utility('extends'))), [this.child, this.parent]).compile(o));
      }
      if (this.traits) {
        _ref = this.traits;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          impl = _ref[_i];
          code.push(new Call(new Value(new Literal(utility('implements'))), [this.child, impl]).compile(o));
        }
      }
      return code.join(";\n" + o.indent);
    };
  ...
  exports.Class = Class = function() {
    ...
    function Class(variable, parent, traits, body) {
      this.variable = variable;
      this.parent = parent;
      this.traits = traits != null ? traits : null;
      this.body = body != null ? body : new Expressions;
      this.boundFuncs = [];
    }
    Class.prototype.children = ['variable', 'parent', 'traits', 'body'];
    ...
    Class.prototype.compileNode = function(o) {
      ...
      this.walkBody(name);
      if (this.parent || this.traits) {
        this.body.expressions.unshift(new Extends(lname, this.parent, this.traits));
      }
  ...
  exports.Trait = Trait = function() {
    function Trait() {
      Trait.__super__.constructor.apply(this, arguments);
    }
    __extends(Trait, Class);
    Trait.prototype.walkBody = function(name) {
      return this.traverseChildren(false, __bind(function(child) {
        var exps, i, node, _len, _ref;
        if (child instanceof Expressions) {
          _ref = exps = child.expressions;
    ...
    Trait.prototype.ensureConstructor = function(name) {
      if (this.ctor) {
        throw SyntaxError("\"constructor\" cannot be defined on a trait.");
      }
      return Trait.__super__.ensureConstructor.apply(this, arguments);
    };
    return Trait;
  }();
