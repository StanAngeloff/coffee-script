(function(){
  ...
  SWITCHES = [['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-o', '--output [DIR]', 'set the directory for compiled JavaScript'], ['-w', '--watch', 'watch scripts for changes, and recompile'], ['-p', '--print', 'print the compiled JavaScript to stdout'], ['-l', '--lint', 'pipe the compiled JavaScript through JSLint'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-e', '--eval', 'compile a string from the command line'], ['-I', '--include [DIR]', 'specify require.paths directory (may be used more than once)', true], ['-r', '--require [FILE]', 'require the library, before executing your script', true], ['--no-wrap', 'compile without the top-level function wrapper'], ['-t', '--tokens', 'print the tokens that the lexer produces'], ['-n', '--nodes', 'print the parse tree that Jison produces'], ['-v', '--version', 'display CoffeeScript version'], ['-h', '--help', 'display this help message']];
  ...
  compile_script = function(source, code, base) {
    var _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, code_opts, expose, file, js, k, o, v;
    o = options;
    code_opts = compile_options(source);
    if ((typeof (_e = o.include) !== "undefined" && _e !== null)) {
      _c = o.include.reverse();
      for (_b = 0, _d = _c.length; _b < _d; _b++) {
        path = _c[_b];
        require.paths.unshift(path);
      }
    }
    if ((typeof (_k = o.require) !== "undefined" && _k !== null)) {
      expose = {
        options: options,
        CoffeeScript: CoffeeScript
      };
      _f = expose;
      for (k in _f) { if (__hasProp.call(_f, k)) {
        v = _f[k];
        (global[k] = v);
      }}
      _h = o.require.reverse();
      for (_g = 0, _i = _h.length; _g < _i; _g++) {
        file = _h[_g];
        require(file);
      }
      _j = expose;
      for (k in _j) { if (__hasProp.call(_j, k)) {
        delete global[k];
      }}
    }
    try {
